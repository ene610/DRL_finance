...
class QNet(nn.Module):
    def __init__(self, state_dim, action_num):
        ...
        self.fc_adv = nn.Linear(in_features=16,
                                out_features=action_num)
        self.fc_val = nn.Linear(in_features=16,
                                out_features=1)
...
class MachinDDDQN():
    ...
    def __init__(self, observe_dim, action_num):
        ...
        self.dddqn = DQN(q_net, q_net_t, t.optim.Adam, nn.MSELoss(reduction="sum"),mode = "double")
        ...
        while episode < max_episodes:
            ...
            while not terminal and step <= max_steps:
                ...
                with t.no_grad():
                    ...
                    action = self.dddqn.act_discrete_with_noise({"state": old_state})
                    reward = float(reward)
            ...
            self.dddqn.store_episode(tmp_observations)
    ...
    def evaluate(self, env):
        ...
        while not terminal and step <= max_steps:
            ...
            with t.no_grad():
                old_state = state
                # agent model inference
                action = self.dddqn.act_discrete({"state": old_state}, use_target=True)
                state, reward, terminal, _ = env.step(action.item())
                reward = float(reward)
                state = t.tensor(state, dtype=t.float32).reshape(1, self.observe_dim)
                total_reward += reward
