...
class MachinDQN():
    ...
    def __init__(self, observe_dim, action_num):
        ...
        self.dqn = DQN(q_net, q_net_t, t.optim.Adam, nn.MSELoss(reduction="sum"),mode = "vanilla")
    ...
    def train(self, env, max_episodes=250):
        ...
        while episode < max_episodes:
            ...
            self.dqn.store_episode(tmp_observations)
    ...
    def evaluate(self, env):
        ...
        while not terminal and step <= max_steps:
            ...
            with t.no_grad():
                old_state = state
                # agent model inference
                action = self.dqn.act_discrete({"state": old_state}, use_target=True)
                state, reward, terminal, _ = env.step(action.item())
                reward = float(reward)
                state = t.tensor(state, dtype=t.float32).reshape(1, self.observe_dim)
                total_reward += reward
