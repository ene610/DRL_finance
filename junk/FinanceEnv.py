...
class FinanceBtcUsdtEnv(gym.Env):
    def __init__(self):
        ...
        self.observation_space = Box(low=np.array([np.finfo(np.float64).min]*393), high=np.array([np.finfo(np.float64).max]*393), dtype=np.float64)
